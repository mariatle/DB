...
SELECT 
    i.inventory_number AS "Номер инвентаря", 
    i.size AS "Размер", 
    i.condition AS "Состояние", 
    e.equipment_name AS "Оборудование", 
    ra.start_date AS "Дата начала аренды", 
    ra.end_date AS "Дата окончания аренды", 
    ra.total_cost AS "Стоимость аренды"
FROM 
    Instance i
JOIN 
...
-- Примеры запросов с операциями UNION и GROUP BY

-- Объединение результатов нескольких запросов (UNION)
SELECT first_name, last_name, phone_number FROM Client
UNION
SELECT first_name, last_name, phone_number FROM Client
ORDER BY first_name;
